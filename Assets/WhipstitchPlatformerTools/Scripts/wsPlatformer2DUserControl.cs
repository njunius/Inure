...
public class wsPlatformer2DUserControl : MonoBehaviour {
	...
	void Update () {
		...
		if (Input.GetKeyDown(KeyCode.W)) { buttonFlags |= ButtonFlags.Jump; }
		if (Input.GetKeyUp(KeyCode.W)) { buttonFlags |= ButtonFlags.ReleaseJump; }//	Feature added by Vincent Chevalier
		if (Input.GetKeyDown(KeyCode.Q)) { buttonFlags |= ButtonFlags.DashLeft; }
		if (Input.GetKeyDown(KeyCode.E)) { buttonFlags |= ButtonFlags.DashRight; }
		if (Input.GetKey(KeyCode.Space)) { buttonFlags |= ButtonFlags.Run; }
		if (Input.GetKeyDown(KeyCode.LeftControl)) { buttonFlags |= ButtonFlags.Grapple; }
